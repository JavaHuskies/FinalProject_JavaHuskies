...
/**
 * Network Admin Dashboard — accessible by networkAdmin, systemAdmin,
 * groupCeo, groupCfo roles.
 *
 * Layout: stats row + tabbed pane (Enterprises | Users | Work Requests)
 *
 * Stub credentials (remove once PersistenceService is live):
 *   username: netadmin  password: Admin1!  role: networkAdmin
 *   username: grpceo    password: Admin1!  role: groupCeo
 */
public class NetworkAdminPanel extends JPanel {
    ...
    private static final Color btnFg = ThemeService.colorTextMuted;
    ...
    private void buildComponents() {
        ...
        statsRow.add(buildStatCard("Enterprises",    "4"));
        statsRow.add(buildStatCard("Organizations",  "8"));
        statsRow.add(buildStatCard("Users",          "24"));
        statsRow.add(buildStatCard("Work Requests",  "12"));
    ...
    private void loadEnterprises() {
        ...
        m.setRowCount(0);
        // TODO: replace with PersistenceService.getInstance().getAllEnterprises()
        ...
        setRowTooltips(enterpriseTable, mockTip);
    }

    private void loadUsers() {
        DefaultTableModel m = (DefaultTableModel) userTable.getModel();
        m.setRowCount(0);
        // TODO: replace with PersistenceService.getInstance().getAllUsers()
        ...
        setRowTooltips(userTable, mockTip);
    }

    private void loadWorkRequests() {
        DefaultTableModel m = (DefaultTableModel) workRequestTable.getModel();
        m.setRowCount(0);
        // TODO: replace with PersistenceService.getInstance().getAllWorkRequests()
        ...
        setRowTooltips(workRequestTable, mockTip);
    }
    ...
    private void updateHeader() {
        titleLabel.setText("Network Admin");
    ...
    private void onNewEnterprise() {
        JOptionPane.showMessageDialog(this,
            "New enterprise form — coming soon.",
            "New Enterprise", JOptionPane.INFORMATION_MESSAGE);
    }
    ...
    private void setRowTooltips(JTable table, String tip) {
        // Tooltips are handled per-row via getToolTipText() override in styledTable()
        // This method is a hook for any additional row-level tooltip logic
    }
}